...
import { KO_BRACKET } from "./tournament.js";

export const MONKEY_BOT_UID = "monkey-bot-amir";
export const MONKEY_BOT_PHOTO = "/monkey.jpg";

// The monkey's knockout bets: a stable pseudo-random scoreline per bracket id, so it
// "bets" on every knockout match too (it only had group bets before). Deterministic
// (seeded by id) so the pick doesn't change on every reload, and low-weighted like a
// realistic scoreline.
const _monkeySeed = s => { let h=2166136261; for(let i=0;i<s.length;i++){ h^=s.charCodeAt(i); h=Math.imul(h,16777619); } return ((h>>>0)%1000)/1000; };
const _monkeyKoBets = {};
for (const k of KO_BRACKET) {
  _monkeyKoBets[k.id] = { home: Math.floor(_monkeySeed(k.id+"h")*3.6), away: Math.floor(_monkeySeed(k.id+"a")*3.6) };
}
