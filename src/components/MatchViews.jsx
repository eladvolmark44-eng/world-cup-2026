...
import { fetchMatchStats, fetchMatchDetail, fetchPlayerClub } from "../utils/api.js";
...
export function MatchLineup({lineup,match,teamNames}){
  ...
  useEffect(()=>{
    if(!lineup)return;
    const all=[...(lineup.home?.starters||[]),...(lineup.home?.subs||[]),...(lineup.away?.starters||[]),...(lineup.away?.subs||[])];
    const ids=[...new Set(all.map(p=>p.id).filter(Boolean))];
    let cancelled=false;
    ids.forEach(id=>{
      fetchPlayerClub(id).then(c=>{
        if(cancelled||!c)return;
        setClubs(prev=>prev[id]?prev:{...prev,[id]:c});
      });
    });
  ...
  const PlayerList=({players})=>players.map((p,i)=>{
    const club=clubs[p.id];
    return(
      <div key={i} className="mdl-player">
        <span className="mdl-jersey">{p.jersey}</span>
        {club?.logo
          ?<img src={club.logo} className="mdl-club-logo" alt="" title={club.name||""}/>
          :<span className="mdl-club-logo mdl-club-ph"/>}
